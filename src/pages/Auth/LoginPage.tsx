...
import { CreditCard, Mail, Lock, Eye, EyeOff, Loader2, ShieldCheck, UserPlus, KeyRound } from 'lucide-react'
...
export default function LoginPage() {
  ...
  const strengthTextColor = { weak: 'text-red-500', medium: 'text-yellow-500', strong: 'text-green-500' }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === 'forgot') {
      ...
      if (result.error) {
        ...
        setSuccess(isZh ? '重置链接已发送到你的邮箱，请查收。' : 'Password reset link sent to your email.')
      }
      setLoading(false)
      return
    }
    ...
    if (mode === 'register') {
      ...
      if (result.alreadyExists) {
        setError(isZh ? '该邮箱已注册，请直接登录或重置密码。' : 'This email is already registered. Please sign in or reset your password.')
      ...
      if (result.error) {
        ...
        if (result.error.includes('Invalid login credentials')) {
          ...
          setError(isZh ? '邮箱尚未验证，请查看收件箱中的确认链接。' : 'Email not confirmed. Please check your inbox for the confirmation link.')
        } else {
          setError(result.error)
        }
      }
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-background p-6">
      ...
      <div className="mb-6 flex flex-col items-center">
        <div className={`mb-4 flex h-16 w-16 items-center justify-center rounded-2xl transition-colors duration-300 ${
          mode === 'register' ? 'bg-green-600' : mode === 'forgot' ? 'bg-amber-500' : 'bg-primary'
        }`}>
          {mode === 'register'
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          {mode === 'register'
            ...
            : mode === 'forgot'
            ? (isZh ? '输入邮箱，我们将发送重置链接' : 'Enter your email to receive a reset link')
      ...
      <div className="w-full max-w-sm">
        <form onSubmit={handleSubmit} className="space-y-3.5">
          ...
          {mode === 'register' && password.length > 0 && (
            <div className="space-y-1.5 px-1">
              <div className="flex items-center justify-between">
                <span className="text-xs text-muted-foreground">{isZh ? '密码强度' : 'Password strength'}</span>
          ...
          <button
            type="submit"
            disabled={loading || (mode === 'register' && passwordStrength === 'weak' && password.length > 0)}
        ...
        <p className="mt-5 text-center text-xs text-muted-foreground">
          {mode === 'register'
            ...
            : mode === 'forgot'
            ? (isZh ? '想起来了？' : 'Remember your password? ')
