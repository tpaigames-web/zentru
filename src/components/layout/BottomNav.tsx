...
const moreItems = [
  { to: '/budget', icon: Wallet, labelKey: 'nav.budget', color: '#f59e0b' },
  { to: '/predictions', icon: TrendingUp, labelKey: 'nav.predictions', color: '#8b5cf6' },
  { to: '/cashback', icon: Gift, labelKey: 'nav.cashback', color: '#22c55e' },
  { to: '/smart', icon: Sparkles, labelKey: 'nav.smartCard', color: '#3b82f6' },
  { to: '/recurring', icon: RefreshCw, labelKey: 'nav.recurring', color: '#ec4899' },
  { to: '/import', icon: FileText, labelKey: 'nav.import', color: '#06b6d4' },
  { to: '/transactions', icon: CreditCard, labelKey: 'nav.transactions', color: '#64748b' },
  { to: '/settings', icon: Settings, labelKey: 'nav.settings', color: '#94a3b8' },
]
  ...
  return (
    <>
        ...
        <div className="fixed inset-0 z-[55] bg-black/40" onClick={() => setShowMore(false)}>
          ...
          >
            ...
            <div className="grid grid-cols-4 gap-3">
              {moreItems.map(({ to, icon: Icon, labelKey, color }) => (
                <button
                  key={to}
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 flex h-14 items-center justify-around border-t bg-background/95 pb-[env(safe-area-inset-bottom)] backdrop-blur supports-[backdrop-filter]:bg-background/60 md:hidden">
        {mainItems.map(({ to, icon: Icon, labelKey, isCenter }) => (
          <NavLink
            key={to}
            to={to}
            end={to === '/'}
